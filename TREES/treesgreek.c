...
void largest(struct BinaryTreeNode*root)
{
...
{
	if (root == NULL)     //node to deleted is leaf node
		return NULL;
	...
	if (x > root->key) {
		...
		if (root->left == NULL && root->right == NULL) {
			...
			if (root->left == NULL) {
				...
				temp = root->left; //cpy node to child n delete child
			}
			free(root);
			return temp;
		}
...
int main()
{
	...
	printf("THE POST ORDER TRAVERSED LIST :");
	postOrder(root);
	printf("\n");

	// Perform pre-order traversal
	printf("THE PRE ORDER TRAVERSED LIST :");
	preOrder(root);
	printf("\n");

	// Perform in-order traversal
	printf("THE IN-ORDER TRAVERSED LIST :");
	inOrder(root);
	printf("\n");
